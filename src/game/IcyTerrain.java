...
/**
 * The main container for the game environment. This class initializes the game components including
 * the TerrainGrid, ObjectSpawner, and GameManager. It orchestrates the game's startup sequence:
 * spawning objects, rendering the initial state, and handing control over to the GameManager for
 * the game loop.
 */
public class IcyTerrain {
  ...
  /**
   * Constructs a new IcyTerrain instance and immediately initializes the game. This constructor
   * sets up the input handler, renderer, object spawner, and the empty terrain grid before calling
   * initializeGame().
   */
  public IcyTerrain() {
  ...
  /**
   * Initializes the game execution sequence. This method performs the following steps: Prints the
   * welcome message. Invokes the ObjectSpawner to populate the grid with Penguins, Hazards, and
   * Food. Renders the initial state of the grid using GridRenderer. Displays information about the
   * generated penguins. Instantiates the GameManager and starts the main game loop. Catches
   * IllegalArgumentException for initialization errors and generic Exception for unexpected runtime
   * errors, printing appropriate error messages.
   */
  ...
  /**
   * Scans the grid for penguins and displays their ID and Type. Iterates through every cell in the
   * TerrainGrid. If a Penguin is found, it prints the penguin's ID (e.g., P1) and its specific type
   * (e.g., Emperor Penguin). This uses a switch expression to map the internal type string to a
   * readable name.
   */
  ...
  /**
   * Sets the game grid. Note: This is primarily used for testing or resetting the game state.
   *
  ...
  /**
   * Closes the InputMaster resource. Should be called when the application is shutting down to
   * prevent resource leaks associated with the java.util.Scanner.
   */
