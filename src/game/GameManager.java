...
public class GameManager {
    ...
    for (int i = 0; i < penguins.size(); i++) {
      ...
      if (items.isEmpty()) {
        ...
        for (int j = 0; j < items.size(); j++) {
          Food f = items.get(j);
          foodInfo.append(f.getNotation()).append(" (").append(f.getWeight()).append(" units)");
  ...
  private void processTurn(Penguin p, int turnNumber) {
    ...
    System.out.println(
        p.getNotation() + " (" + capitalizeFirst(p.getType()) + " Penguin) is preparing to move.");
  ...
  private void handlePlayerTurn(Penguin p) {
    ...
    if (!p.isAbilityUsed()) {
      useAbility =
          inputMaster.getYesNoInput(
              "Will " + p.getNotation() + " use its special action? Answer with Y or N: ");
    } else {
    ...
    if (useAbility && !p.isAbilityUsed()) {
      ...
      if (p instanceof RoyalPenguin royal) {
        ...
        Direction specialDir =
            inputMaster.getDirectionInput(
                "Which direction for the special move? Answer with U (Up), D (Down), L (Left), R (Right): ");

        System.out.println(
            "[DEBUG] Input received: " + specialDir + ". Performing move..."); // DEBUG 4
