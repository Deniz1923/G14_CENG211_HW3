...
public class ObjectSpawner {
    ...
    private Penguin generateRandomPenguin(Position pos) {
        ...
        switch(selected){
            case ROYAL:
                return new RoyalPenguin(pos);
            // Assuming other constructors exist similarly
            case EMPEROR:
                return new EmperorPenguin(pos);
            case KING:
                return new KingPenguin(pos);
            case ROCKHOPPER:
                return new RockhopperPenguin(pos);
            default:
                return new RoyalPenguin(pos);
        }
    }
    ...
    private ITerrainObject generateRandomHazard(Position position) {
        ...
        switch (pick) {
            case 0: return new models.hazards.LightIceBlock(position);
            case 1: return new models.hazards.HeavyIceBlock(position);
            case 2: return new models.hazards.SeaLion(position);
            case 3: return new models.hazards.HoleInIce(position);
            default: return new models.hazards.HoleInIce(position);
        }
    }
    ...
    private Food generateRandomFood(Position position) {
        ...
        // Pick random Weight between 1 and 5
        int randomWeight = random.nextInt(5) + 1;

        return new Food(selectedType, position);
    }
}
